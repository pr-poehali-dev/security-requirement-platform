...
import { dashboardApi, type DashboardStats } from "@/api/index";

const actionColor = (t: string) =>
  ({ create: "var(--success)", update: "var(--amber)", delete: "var(--danger)", action: "var(--steel)" }[t] || "var(--text-dim)");
...
export default function Dashboard() {
  ...
  const STATS = [
    { label: "Требований в реестре",   value: stats?.requirements  ?? "…", icon: "FileText",      color: "var(--amber)",     path: "/requirements" },
    { label: "Архитектур",             value: stats?.architectures ?? "…", icon: "Network",       color: "var(--steel)",     path: "/architectures" },
    { label: "Шаблонов",               value: stats?.templates     ?? "…", icon: "LayoutTemplate",color: "var(--success)",   path: "/templates" },
    { label: "Активных пользователей", value: stats?.active_users  ?? "…", icon: "Users",         color: "var(--text-dim)",  path: "/users" },
  ];

  const sevDist = stats ? [
    { label: "Критических", count: stats.req_by_severity["critical"] ?? 0, color: "var(--danger)",  pct: 0 },
    { label: "Высоких",     count: stats.req_by_severity["high"]     ?? 0, color: "hsl(28 85% 60%)", pct: 0 },
    { label: "Средних",     count: stats.req_by_severity["medium"]   ?? 0, color: "var(--amber)",   pct: 0 },
    { label: "Низких",      count: stats.req_by_severity["low"]      ?? 0, color: "var(--success)", pct: 0 },
  ].map((s, _, arr) => {
    const total = arr.reduce((a, b) => a + b.count, 0) || 1;
    return { ...s, pct: Math.round(s.count / total * 100) };
  }) : [];

  const archByStatus = stats?.arch_by_status ?? {};
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-sec mt-0.5">
            {new Date().toLocaleDateString("ru-RU", { day: "numeric", month: "long", year: "numeric" })} · Служба информационной безопасности
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs text-sec bg-surface-1 border border-line px-3 py-1.5 rounded">
          <span className="status-dot bg-amber animate-pulse-amber" />
      ...
      <div className="grid grid-cols-4 gap-4">
        {STATS.map((s, i) => (
          <button
            ...
            className="bg-surface-1 border border-line rounded p-4 text-left hover:border-amber/30 transition-colors animate-fade-in"
          ...
          >
            ...
            <div className="text-2xl font-semibold text-foreground mb-1">{String(s.value)}</div>
          </button>
        ))}
      </div>
        ...
        <div className="bg-surface-1 border border-line rounded p-4 animate-fade-in stagger-3">
          ...
          <div className="mt-5 pt-4 border-t border-line">
            ...
            <div className="flex gap-2">
              {[
                { label: "Утверждено",  key: "approved", color: "var(--success)" },
                { label: "На проверке", key: "review",   color: "var(--amber)" },
                { label: "Черновик",    key: "draft",    color: "var(--text-dim)" },
              ].map(s => (
                <div key={s.label} className="flex-1 bg-surface-2 rounded p-2 text-center">
                  <div className="text-base font-semibold" style={{ color: s.color }}>{archByStatus[s.key] ?? 0}</div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="col-span-2 bg-surface-1 border border-line rounded p-4 animate-fade-in stagger-4">
          ...
          {RECENT_ACTIVITY.map((a, i) => (
            <div key={i} className="flex items-center gap-3 py-2.5 border-b border-line/40 last:border-0">
              ...
              <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: actionColor(a.type) }} />
        ...
        <div className="bg-surface-1 border border-line rounded p-4 animate-fade-in stagger-5">
          ...
          <div className="space-y-2">
            {[
              { label: "Добавить требование",    icon: "FilePlus",      color: "var(--amber)",    path: "/requirements" },
              { label: "Создать архитектуру",    icon: "PlusSquare",    color: "var(--steel)",    path: "/architectures" },
              { label: "Применить шаблон",       icon: "LayoutTemplate",color: "var(--success)",  path: "/templates" },
              { label: "Добавить технологию",    icon: "Cpu",           color: "var(--amber)",    path: "/technologies" },
              { label: "Пользователи",           icon: "UserPlus",      color: "var(--text-dim)", path: "/users" },
            ].map(a => (
              <button
                key={a.label}
